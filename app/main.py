...
@app.get("/")
def root(): return {"system":"CCIOS V9 FINAL CORE - LOVED","today":"32 orders, 8 OOS, 15 reviews waiting","visual_commerce":"NO COMPETITOR LIVE","docs":"/docs","status":"LIVE ✅","backend":"CCIOS V9 FIXED - REPLACE ALL VERSION"}
...
def today(brand_id: int): return {"brand_id":brand_id,"orders_new":32,"oos":8,"reviews_pending":15,"campaigns_ending":5,"publish_failed":3,"yesterday_rm":12540,"growth":"+18%","nova":"Restock RAV Bifold Wallet MB-001 Sales +42% 3 days left"}

@app.get("/visual/{brand_id}")
def visual(brand_id: int): return {"brand_id":brand_id,"homepage_score":89,"before_ctr":"1.2%","after_ctr":"3.8%","before_res":"800x400 low res","after_res":"1920x600 high-res lifestyle","category_recommend":"kasut kulit 22k -> beg 18k -> dompet 12k","layout_rules":"Premium minimal white space editorial"}
...
def consultant(q: dict): return {"answer":"Overall 94/100 healthy. Today: 32 orders new, 8 OOS, 15 reviews pending. Top action: Fix hero banner CTR 1.2->3.8% + Restock RAV001. Impact RM8,200/mo."}
...
def seed(db: Session=Depends(get_db)):
    ...
    for i in range(1,16):
        db.add(Review(brand_id=b1.id if i%2==0 else b2.id, product=f"Wallet {i}", rating=5 if i%3!=0 else 4, comment="Kasut kulit sangat cantik kualiti premium" if i%2==0 else "Very nice design!", status="pending"))
    db.commit()
    return {"seeded":True}
